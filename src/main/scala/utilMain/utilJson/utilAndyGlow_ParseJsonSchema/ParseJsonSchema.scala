...
/**
 * CODE COPIED FROM = https://github.com/statisticallyfit/AndyGlow_scala-jsonschema/blob/master/modules/parser/src/main/scala/com/github/andyglow/jsonschema/ParseJsonSchema.scala
 *
 * REASON = the makeType function is private to jsonschema package so cannot import it dierctly into my project, so must copy it here.
 */
...
object ParseJsonSchema {
	...
	def apply(x: String): Try[SchemaJson_AndyGlow[_]] = apply(new ByteArrayInputStream(x.getBytes))
	
	def apply(x: InputStream): Try[SchemaJson_AndyGlow[_]] = ParseJson(x) flatMap {
		apply(_)
	}
	
	def apply(x: Value, checkSchema: Boolean = true): Try[SchemaJson_AndyGlow[_]] = x match {
		case o@obj(fields)
	...
	/*private[jsonschema] */def makeType(x: obj): Try[SchemaJson_AndyGlow[_]] = {
		...
		def makeStrOrEnum: Try[SchemaJson_AndyGlow[String]] = x.value.arr("enum") match {
			...
			case Some(arr) =>
				tpe.map(_.toLowerCase) match {
					case None | Some("string") => Success {
						`enum`(`string`, arr.toSet)
					}
					case Some("integer") => Success {
						`enum`(`integer`, arr.toSet)
					}
					case Some("boolean") => Success {
						`enum`(`boolean`, arr.toSet)
					}
					case Some("number") => Success {
						`enum`(`number`[Double], arr.toSet)
					}
					case Some(x) => Failure {
						InvalidEnumType(x)
					}
		...
		def makeObj = x.value.obj("patternProperties") match {
			...
			case _ =>
				...
				x.value.obj("properties").map {
					_.value
				}.toSuccess("properties is not defined") flatMap { props =>
	...
	/*private[jsonschema]*/ def parseFormat(x: String): Option[SchemaJson_AndyGlow.`string`.Format] = {
		import SchemaJson_AndyGlow.`string`.Format._
