...
object ConverterAutoschema_ClassToJsonSchema extends App {
	...
	import conversionsOfSchemaADTs.avro_json.data_avrojsonPairs.data_Velo.managed.EnumColors_fromAvroToClassBySkeuo._

	val colorsEnumJsonSchema = AutoSchema.createSchema[TheColorsEnum]
	println(s"colors enum = ${colorsEnumJsonSchema.toString()}")
