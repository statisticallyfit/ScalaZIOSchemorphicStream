...
object instances {
  ...
  implicit val avroSchemaArbitrary: Arbitrary[ApacheAvroSchema] = Arbitrary {
    val primitives: Gen[ApacheAvroSchema] = Gen.oneOf(
      List(
        ApacheAvroSchema.Type.STRING,
        ApacheAvroSchema.Type.BOOLEAN,
        ApacheAvroSchema.Type.BYTES,
        ApacheAvroSchema.Type.DOUBLE,
        ApacheAvroSchema.Type.FLOAT,
        ApacheAvroSchema.Type.INT,
        ApacheAvroSchema.Type.LONG,
        ApacheAvroSchema.Type.NULL
      ).map(ApacheAvroSchema.create)
    )

    val arrayOrMap: Gen[ApacheAvroSchema] =
      Gen.oneOf(primitives.map(ApacheAvroSchema.createMap), primitives.map(ApacheAvroSchema.createArray))

    val union: Gen[ApacheAvroSchema] =
      Gen.nonEmptyContainerOf[Set, ApacheAvroSchema](primitives).map(l => ApacheAvroSchema.createUnion(l.toList.asJava))

    def field(name: String): Gen[ApacheAvroSchema.Field] =
      for {
        schema <- Gen.oneOf(primitives, arrayOrMap, union)
        doc <- nonEmptyString
      } yield new ApacheAvroSchema.Field(name, schema, doc, null.asInstanceOf[Any])

    val record: Gen[ApacheAvroSchema] = (
      nonEmptyString,
      nonEmptyString,
      nonEmptyString,
      ...
      ApacheAvroSchema.createRecord(name, doc, namespace, false, fields.asJava)
    }
