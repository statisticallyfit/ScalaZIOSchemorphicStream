...
class Convert_ApacheAvroSchema_to_SkeuomorphAvroSchema extends Specification with ScalaCheck {
    ...
    Algebra {
      ...
      case SkeuomorphAvroSchema.TRecord(name, namespace, _, doc, fields) =>
        (sch.getName should_== name)
          ...
          .and(sch.getFields.asScala.toList.map(f ⇒ f.)
  ...
  // TODO left off here - going to look at this version of the implementation ot see how they return the schema as result using hylo = https://github.com/wi101/recursion-schemes-lc2018/blob/master/src/main/scala/solutions/2-avro.scala
