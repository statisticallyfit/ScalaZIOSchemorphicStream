...
class AvroSchema_Apache_Zio_RoundTrip_Spec extends Specification with ScalaCheck {
	...
	val tangeloApacheAvroSchema: Either[String, SchemaAvro_Apache] = AvroCodec.encodeToApacheAvro(tangeloSchema_Zio)
	val tangeloApacheAvroString: Either[String, String] = AvroCodec.encode(tangeloSchema_Zio)
	
	println(s"Schema_Zio tangelo = $tangeloSchema_Zio")
	...
	val arrayZio2: Either[String, Schema_Zio[_]] = AvroCodec.decodeFromApacheAvro(arrayIntSchema_Apache)
